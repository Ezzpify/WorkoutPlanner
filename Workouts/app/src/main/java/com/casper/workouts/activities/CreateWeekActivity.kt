package com.casper.workouts.activities

import android.app.Activity
import android.content.Intent
import android.os.Bundle
import android.view.View
import androidx.appcompat.app.AppCompatActivity
import com.casper.workouts.R
import kotlinx.android.synthetic.main.activity_create_week.*

class CreateWeekActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_create_week)
    }

    fun onSaveWeekButtonClicked(view: View) {
        var error = false
        var errorText = getString(R.string.required)

        val weekName = week_name.text.toString().trim()
        val weekDesc = week_description.text.toString().trim()

        if (weekName.isEmpty()) {
            week_name.error = errorText
            error = true
        }

        if (error) {
            return
        }

        val replyIntent = Intent()
        replyIntent.putExtra(EXTRA_REPLY_NAME, weekName)
        replyIntent.putExtra(EXTRA_REPLY_DESC, weekDesc)
        setResult(Activity.RESULT_OK, replyIntent)
        finish()
    }

    companion object {
        const val EXTRA_REPLY_NAME = "REPLY_NAME"
        const val EXTRA_REPLY_DESC = "REPLY_DESC"
    }
}